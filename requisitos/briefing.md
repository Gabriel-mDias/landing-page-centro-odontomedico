---
tipo_documento: briefing
versão_esquema: 2
nome_cliente: Centro Odontomédico
nome_cliente_estado: confirmado
nome_cliente_fonte: Instagram oficial e manifest
nome_cliente_consultado_em: 2026-09-19
nome_cliente_contexto: Identidade consolidada nas fontes oficiais coletadas
nome_cliente_autorização: aprovado
arquétipo_primário: institucional
arquétipo_secundário: não_aplicável
objetivo_de_conversão: agendamento_whatsapp
cta_primário: Agendar Consulta
tipo_cta_primário: falar_com_empresa
conversão_estado: confirmado
conversão_fonte: Manifest
conversão_consultado_em: 2026-09-19
conversão_contexto: WhatsApp principal confirmado no intake
conversão_autorização: aprovado
contato_principal_estado: confirmado
seo_estado: confirmado
---

# Briefing — Centro Odontomédico

## Controle e conversão

- Responsável pela aprovação: Usuário
- Data de aprovação da revisão: 2026-09-19
- Revisão aprovada: a jornada do cuidado passa a usar quatro painéis editoriais em zigue-zague, em fluxo normal, sem cena sticky, órbitas ou progressão controlada por scroll.
- Arquétipo primário: institucional, apresentando posicionamento, capacidade clínica e reputação confirmada.
- Objetivo: agendamentos pelo WhatsApp.
- CTA único: “Agendar Consulta”.

## Negócio, público e oferta

| Campo | Valor | Fonte | Data | Estado | Autorização |
|---|---|---|---|---|---|
| Nome | Centro Odontomédico | Instagram/manifest | 2026-09-19 | confirmado | aprovado |
| Nicho | Odontologia multidisciplinar | Manifest | 2026-09-19 | confirmado | aprovado |
| Local | Bom Jesus do Itabapoana — RJ | Manifest | 2026-09-19 | confirmado | aprovado |
| Histórico | Mais de 19 anos | Bio oficial | 2026-09-19 | confirmado | aprovado |
| Equipe | 11 profissionais no catálogo fornecido | Mídia fornecida | 2026-09-19 | confirmado | aprovado |
| Público | Pessoas que procuram atendimento odontológico multidisciplinar | Briefing | 2026-09-19 | inferido | aprovado |

## Identidade visual aprovada

| Item | Especificação | Origem/contexto | Estado | Autorização |
|---|---|---|---|---|
| Fundo | `#0A0A0A` preto mineral | Direção editorial clínica | inferido | aprovado |
| Superfície | `#151515` grafite | Direção editorial clínica | inferido | aprovado |
| Acento | `#C6A15B` dourado fosco, único acento | Direção editorial clínica | inferido | aprovado |
| Texto | `#F2F0EA` marfim | Direção editorial clínica | inferido | aprovado |
| Tipografia | Manrope variável auto-hospedada | Distribuição Fontsource, OFL | inferido | aprovado |
| Raio | 12px | Sistema visual | inferido | aprovado |

## Repertório verbal

- Frase central literal e aprovada: “Invista no seu sorriso!”
- Tom: clínico, seguro, direto e acolhedor.
- Não publicar credenciais, horários, e-mail, resultados ou claims sem confirmação.

## Ativos e uso

| Ativo | Arquivo | Estado | Escopo |
|---|---|---|---|
| Hero | `media/Instagram/hero_1/hero_1.mp4` | confirmado | autorizado para a página |
| Jornada | quatro fotografias de `media/home`: retrato do Dr. Mayson, recepção e dois consultórios | confirmado | acervo indicado pelo usuário para a seção; seleção aprovada por tópico nesta revisão |
| Equipe | `media/funcionarios/` | confirmado | autorizado para a página |
| Casos clínicos | oito conjuntos em `media/Instagram/` | bloqueado | preparar tecnicamente; não publicar sem autoria, CRO e TCLE por caso |
| Símbolo | `media/Instagram/logo_1/logo_2.png` | confirmado | reconstrução vetorial fiel autorizada |

## Contatos confirmados

- WhatsApp: +55 22 99815-5861.
- Telefone: +55 22 3831-1733.
- Endereço: Rua XV de Novembro, 360, Centro, Bom Jesus do Itabapoana — RJ, 28360-000.
- Instagram: @centroodontomedico_.
- Formulário sem chave externa: prepara o atendimento pelo WhatsApp, preservando nomes e ordem dos campos.

## Restrições

- Casos clínicos permanecem ocultos até a regularização individual.
- Sem domínio, analytics, serviço de formulário ou publicação externa nesta entrega.
- `SITE_CONFIG` é a fonte única de contatos e mídia publicada.
